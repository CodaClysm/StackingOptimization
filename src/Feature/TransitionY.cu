...
__global__ void kernel(State *states, double *returnVals)
{
    ...
    Here, a transition occurs for each edge of an occupied cell that is adjacent to an unoccupied cell along the Y axis. The walls count as occupied cells. 
    */
...
vector<double> TransitionY::calculateFeature(vector<State> s)
{
